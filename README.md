...
**RC-1 status:** dependency pins (`next`, `react`, `react-dom`,
`tailwindcss`) have been updated to verified-current versions as of
August 2026 — see the RC-1 status report for rationale, including a
real security-relevant finding (the previous React pin predated
patched RSC security advisories). The actual `npm install` / `lint` /
`build` / `typecheck` / lockfile generation still needs to run in an
environment with registry access, which this sandbox doesn't have.
